...
MALLS = [
    {
        ...
        "fields": [
            ...
            {"key": "shop_url", "label": "자사몰 주소", "type": "text",
             "help": "예: https://operations.co.kr — 셋팅 화면에서 상품명을 누르면 이 주소의 "
    ...
    },
    {
        ...
        "fields": [
            ...
            # 스마트스토어와 같은 분류 — 렌탈 주문이 OWS로 들어오면 판매 출고 사고가 난다
    ...
    },
    {
        ...
        "fields": [
            ...
            # 같은 스토어에 렌탈(RMS)·판매(OWS) 상품이 함께 있다 — 렌탈 주문이 OWS로
            ...
            {"key": "sale_product_ids", "label": "판매 상품번호 (항상 수집)", "type": "text",
             "help": "쉼표로 구분. 상품명에 '렌탈'이 들어가도 판매 상품이면 여기 등록하세요 — "
                     "이 목록이 자동 제외보다 우선합니다."},
            {"key": "store_url", "label": "스토어 주소", "type": "text",
             "help": "셋팅 화면에서 상품명을 누르면 이 스토어의 상품 페이지로 바로 갑니다. "
                     "비워 두면 https://smartstore.naver.com/ExampleShare 를 씁니다"
                     "(스토어를 옮겼거나 다른 스토어를 쓰면 여기에 넣으세요)."},
        ],
    },
    {
        "code": "kakao",
...
COMMON_FIELDS = [
    ...
    # ★보내는 시점(2026-09-08 대표 "송장 뽑았는데 고도몰에 왜 안 들어가 있지"): 대표가 몰 관리자에서
    #   손으로 송장을 넣던 시점이 '뽑은 직후'다. 그래서 기본은 발급 즉시. 저장된 적 없는 칸(옛 설정)은
    #   켜진 것으로 본다(default) — 끄면 예전처럼 출고 확인 때 보낸다(그 경로는 그대로 남아 있다).
    {"key": "push_on_issue", "label": "송장 발급 즉시 전송", "type": "bool", "default": True,
     "help": "켜면 송장을 발급(출력)하는 순간 이 몰에 송장번호를 올립니다. 끄면 출고 확인 때 보냅니다. "
             "몰은 송장을 받는 순간 주문을 '배송중'으로 바꾸고 고객에게 안내를 보냅니다."},
