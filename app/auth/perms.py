...
MENUS = [
    {
        ...
        "perms": [("purchase.edit", "등록·수정 (읽기만 하려면 끄기)"),
                  # 금액 분리(2026-08-27 대표) — 매입 접근과 금액 열람은 별도. 없으면
                  # 매입가·전표 금액이 •••로 가려지고 미지급·매출 화면이 안 열린다.
                  ("purchase.money", "금액 열람 (매입가·전표 금액·미지급·매출)")],
    },
    {
    ...
    },
    {
        ...
        "perms": [
            ...
            # ★리포트 메뉴는 설정 ▸ 매출/실적으로 통합(2026-08-31 대표) — 권한 코드는
            #   그대로 살아 있어야 한다(기존 부여분·백엔드 require 15곳이 이 코드를 쓴다).
            ("reports.view", "매출·실적 열람(옛 리포트)"),
            ("audit.view", "감사 로그 조회"),
        ],
    },
]
...
def can(perm):
    """403 없이 보유 여부만 — 응답에서 금액을 가릴지 갈림길에 쓴다."""
    u = getattr(g, "user", None)
    if not u:
        return False
    if u["is_admin"]:
        return True
    return perm in getattr(g, "perms", set())
